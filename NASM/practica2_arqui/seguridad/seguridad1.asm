...
;parametro1-> lista de numeros (const_1)
;parametro2-> No digitos que contiene const_1
;parametro3-> posicion correspondiente que le toca almacenarse al numero
;parametro4-> lista donde se almacenan los digitos en hexa   solo caben 50
%macro decimal_to_hexa 4
  pusha
    mov bx,%1       ;movemos const_1
    mov si,0        ;empezamos por el inicio
    mov di,[%3]     ;asigno el contador a di
    ;add di,30h      ;lo convierto a decimal
   ...
   ;mov ah,02h;
    ...
    mov dx,000Ah    ;es numero 10 en decimal y sirve par la conversion

    mov cl,%2       ;movemos a cl el conteo que debe recorrer
    ...
    ;  add si,30
      mov bx,%4         ;le asignamos como base la lista de los numeros
    ...
    %%hay_tres_digitos:
      mostrar_cadena tres_digitos
    jmp %%fin_macro

    %%hay_dos_digitos:
      mostrar_cadena dos_digitos
    jmp %%fin_macro
...
%macro analizar_cadena 1
  pusha
    ...
    %%BUCLE_PRINCIPAL:
     mov al,[bx+di]      ;movemos el caracter actual al registro AL
   ...
   %%coma:
  ;  mov byte[cont_posicion_lst],1
     decimal_to_hexa const_1, cont_digito,cont_posicion_lst,lst_valores
   jmp %%fin_macro


   %%punto_coma:
      ;notificar al usuario que se analizo la cadena
   jmp %%fin_macro
...
segment .data
  ...
  lst_valores    times 50 db 0        ;almacenara como maximo 50 valores

  digito_tmp     db     0

  flag_caracter_digito  db   0

  cont_posicion_lst  db 30h

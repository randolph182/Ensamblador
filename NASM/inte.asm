; /***********************************************************************/
;****       Estudiantes de Universidad de san carlos de Guatemala     ****
;****       Proyecto_2 Arquitectura de Computadores y ensamblador 1   ****
;*************************************************************************
;**** El objetivo de este proyecto es implementar una comunicacion    ****
;**** por puerto serial.Para lo cual implemntamos una transferencia   ****
;**** paralela. haciendo uso de IN y OUT.Cabe resaltar que se debe    ****
;**** habilitar el serial en DOSBOX  con los Serial1.... ademas cuando ***
;**** se utiliza IN en cado de arduino despues de enviar el caracter   ***
;**** debemos enviar tambien el dato para borrar lo que hay en el puerto *
;*** en este caso  un  Serial1.print(0x0d). EL PROYECTO   en comunicacion*
;*** con arduino se comunica para manejar un cubo de leds en 3D para *****
;** para mostrar en tiempo real lo que se envia desde ensamblador   ******
;/*************************************************************************/


;*****************************************   METODOS    *****************************************

;macro que incicializa el puerto
;parametro1--> la direccion del puerto 00h(com1), 01(com2) etc
%macro inicializar_puerto 1
  mov dx,%1 ;puerto com1,com2 etc
  mov ah,00h ;para inicializar el puerto
  mov al,11100011b ;configuracion del puerto 9600 bd paridad:n 1 bit de parada 8 bits de envio
  int 14h
%endmacro

;se va a estar enviando por el pureto com  1 porque previamente en la macro:
;iniciacilar_puerto configura el puerto com1 en dx
;parametro1 --> byte a enviar
%macro envio_serial  1
 pusha
    mov ah,01h
    mov al,%1
    int 14h
...
;metodo que utiliza la comunicacion paralela  OUT e IN para interactuar con dispositivos Externos
;NOTA se debe de instanciar el perto que se le pasa a DX en Dosbox con los comandos serial1=directserial realport:(puerto)
;por defecto ese metodo utiliza el puerto COM1 Y OJO hay que dar un retardo a conveniencia despues del envio por OUT
;p1--> puerto p2-->caracter a enviar
%macro envio_8_bits_arduino 1
  pusha
    mov dx,03F8h
    mov al,%1
    out dx,al
...
;macro que asigna valoes nuevos a una posicion especifica del snake
;p1-> valor p2->posicon
...
;parametro 1  CX high
;parametro2 DX LOW
;CX:DX  tiempo en microsegundos hexadecimal
%macro delay 2
  pusha
...
%macro enviar_texto_arduino 0
  pusha
    xor si,si
    inicializar_puerto 0000h
    enviar_inicio:
      xor bx,bx
...
%macro limpiar_ruta 0
  pusha
    ...
    cmp si,70
    je limpiar_ruta_fin
    jmp limpiar_ruta_inicio
    limpiar_ruta_fin:
  popa
%endmacro
%macro limpiar_buffer 0
  pusha
    xor si,si
    mov al,'$'
    limpiar_buffer_inicio:
    mov [buffer+si],al
    inc si
    cmp si, 256
    je limpiar_buffer_fin
    jmp limpiar_buffer_inicio
    limpiar_buffer_fin:
  popa
%endmacro

%macro espera_activacion 0
  mov dx, 03f8H
  ;in al, dx
  delay 002Dh,0C6C0h
  mov al, 97
  cmp al, 97
   je CONTROL_MOVIL_MENU
%endmacro
org 100h
...
msg_reposo_movil   DB 10, 13
        DB 201,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205, 187, 10, 13
        DB 186,"                                                                            ",186,  10, 13
        DB 186,"      CCCCCC    OOOOO    NN    N   TTTTTTT   RRRRRR     OOOOO    LLL        ",186,  10, 13
        DB 186,"     CCCCCCC   OOOOOOO   NNN   N   TTTTTTT   RRRRRRR   OOOOOOO   LLL        ",186,  10, 13
        DB 186,"     CCC       OO   OO   NNNN  N      TT     RR   RR   OO   OO   LLL        ",186,  10, 13
        DB 186,"     CCC       OO   OO   NN NN N      TT     RRRRRRR   OO   OO   LLL        ",186,  10, 13
        DB 186,"     CCC       OO   OO   NN  NNN      TT     RRRRRR    OO   OO   LLL        ",186,  10, 13
        DB 186,"     CCC       OO   OO   NN   NN      TT     RR RR     OO   OO   LLL        ",186,  10, 13
        DB 186,"     CCCCCCC   OOOOOOO   NN    N      TT     RR  RR    OOOOOOO   LLLLLLL    ",186,  10, 13
        DB 186,"      CCCCCC    OOOOO    NN    N      TT     RR   RR    OOOOO    LLLLLLL    ",186,  10, 13
        DB 186,"                                                                            ",186,  10, 13
        DB 186,"       MM       MM    OOOOOO   VV              VV   IIIIIIII   LLL          ",186,  10, 13
        DB 186,"       MMM     MMM   OOOOOOOO   VV            VV    IIIIIIII   LLL          ",186,  10, 13
        DB 186,"       MM MM MM MM   OO    OO    VV          VV        II      LLL          ",186,  10, 13
        DB 186,"       MM  MMM  MM   OO    OO     VV        VV         II      LLL          ",186,  10, 13
        DB 186,"       MM       MM   OO    OO      VV      VV          II      LLL          ",186,  10, 13
        DB 186,"       MM       MM   OO    OO       VVVVVVVV           II      LLL          ",186,  10, 13
        DB 186,"       MM       MM   OOOOOOOO        VVVVVV         IIIIIIII   LLLLLLLL     ",186,  10, 13
        DB 186,"       MM       MM    OOOOOO          VVVV          IIIIIIII   LLLLLLLL     ",186,  10, 13
        DB 186,"                                                                            ",186,  10, 13
...
;=============================================================================== JUGAR ================================================================
  JUGAR:
    ...
    bucle_juego:
      ...
      ;== PROCESO QUE ENVIA LA INFORMACION DEL SNAKE ES DECIR LA MATRIZ========
      xor si,si
      inicializar_puerto 0000h
      bucle_env_dato_juego:
        call verificar_movimiento
        xor bx,bx
        mov bl,[matriz_juego+si] ;muevo la informacion de la matriz en la posicion a bl
        envio_serial bl           ;enviamos bl la cual contiene el byte de informacion
        cmp si,63
        je fin_bucle
        inc si
      jmp bucle_env_dato_juego
      ;====================================================================
     fin_bucle:
    ...
    snake_colision:
      modo_video_normal
      imprimir msg_perdio
      SNAKE_CICLO_COLISION:
      mov ah,01h
      int 21h
      cmp al, 27
      jne SNAKE_CICLO_COLISION
...
VERIFICACION_CORRECTA: ;si la ruta no es valida no es correcta pide ingreso de nuevo INTRODUCIR_RUTAUT2
  ...
  enviar_texto_arduino

  xor ax,ax
  mov ah,'@'
  envio_8_bits_arduino ah
continuacion_verificacion_carrecta:
  limpiar_pantalla
  imprimir menu2
  ...
  cmp al,35h          ;opcion 5
  je regreso_aMenu
jmp continuacion_verificacion_carrecta
...
pintar_margen_juego:
  ...
  pintar_secuencia_horizontal 39,99,219,0036h;000Fh ;linea superior
  pintar_secuencia_horizontal 160,99,219,0036h;000Fh ;linea inferior
                            ;col,fila_ini,fila_fin,color
  pintar_secuencia_vertical 99,39,159,0036h;000Fh   ;linea_izqs
  pintar_secuencia_vertical 220,39,160,0036h;000Fh   ;linea_der
ret
